...
<template>
  <div class="relative h-full w-full overflow-hidden">
    <CircleText :text-list="expandNum(60, 0, '秒')" radius="53em" :current="`${nzh.encodeS(date[6])}秒`.replace(/^一十/, '十')" />
    <CircleText :text-list="expandNum(60, 0, '分')" radius="43em" :current="`${nzh.encodeS(date[5])}分`.replace(/^一十/, '十')" />
    <CircleText :text-list="expandNum(24, 0, '时')" radius="33em" :current="`${nzh.encodeS(date[4])}时`.replace(/^一十/, '十')" />
    <CircleText :text-list="['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六']" radius="25em" :current="date[3]" />
    <CircleText :text-list="expandNum(day, 1, '日')" radius="15em" :current="`${nzh.encodeS(date[2])}日`.replace(/^一十/, '十')" />
    <CircleText :text-list="expandNum(12, 1, '月')" radius="7em" :current="date[1]" />
